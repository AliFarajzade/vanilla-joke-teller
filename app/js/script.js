...
const getJokes = async function () {
  ...
  try {
    ...
    if (!response.ok) throw new Error('Could not get data from the API.');

    const data = await response.json();

    // checking for two-part jokes.
    if (data.joke) joke = data.joke;
    else joke = `${data.setup}...${data.delivery}`;
    toggleBtn();
    tellJoke(joke);
  } catch (error) {
    console.error(`getJokes: ${error.message}`);
  }
};
...
const tellJoke = function (joke) {
  VoiceRSS.speech({
    key: '75901f3ee4cf4ec586924911cfcf7ae9',
    src: joke,
    hl: 'en-us',
    v: 'Linda',
    r: 0,
